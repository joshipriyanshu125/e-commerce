...
const ProductDetails = () => {
  ...
  // Try to select from store first; if products not loaded yet, fetch them
  useEffect(() => {
  ...
  // If not in store, trigger a full fetch then re-select
  useEffect(() => {
  ...
  // Set default color/size once product is loaded
  useEffect(() => {
    if (selectedProduct) {
  ...
  // Normalize images: always an array of URL strings
  const images = (selectedProduct.images || []).map(img =>
    typeof img === 'string' ? img : img?.url
  ).filter(Boolean)

  const fallbackImg = 'https://images.unsplash.com/photo-1512436991641-6745cdb1723f?auto=format&fit=crop&w=900&q=80'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start pb-20">
        ...
        <div className="lg:col-span-7 space-y-4 lg:sticky lg:top-28 lg:self-start">
          <div className="aspect-square bg-atelier-cream border border-atelier-lightgray/30 overflow-hidden relative rounded-xl">
            <img 
              src={images[activeImageIdx] || fallbackImg}
            ...
            <div className="flex gap-3">
              {images.map((img, idx) => (
                <button
                  ...
                  className={`h-16 w-16 bg-atelier-cream border overflow-hidden flex-shrink-0 ${
