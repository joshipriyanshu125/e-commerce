...
export default function SearchModal({ onClose }) {
  ...
  if (showVisualSearch) {
    return <VisualSearchModal onClose={() => setShowVisualSearch(false)} />
  }
  ...
  return <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm p-4 sm:p-10" onMouseDown={onClose}>
    <div className="mx-auto max-w-3xl bg-atelier-beige shadow-2xl p-6 sm:p-8" onMouseDown={e => e.stopPropagation()}>
      <div className="flex items-center border-b border-atelier-dark gap-3 pb-3">
        <Search size={20}/>
        <input ref={inputRef} value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && search()} placeholder="Search products, brands, categories and styles" className="w-full bg-transparent font-serif text-xl outline-none"/>
        <button
          onClick={() => setShowVisualSearch(true)}
          title="Search by image (AI Vision)"
          className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/30 text-amber-700 hover:text-amber-900 transition-all font-mono text-xs flex-shrink-0"
        >
          <Camera size={16} />
      ...
      <div className="mt-4 flex items-center justify-between">
        {query ? (
          <button onClick={() => search()} className="btn-atelier-dark text-xs">Search for “{query}”</button>
        ) : <span />}

        <button
          onClick={() => setShowVisualSearch(true)}
          className="flex items-center gap-1.5 text-xs text-amber-800 font-mono hover:underline"
        >
          <Sparkles size={13} className="text-amber-600" />
          <span>Have an outfit photo? <b>Try AI Image Search</b></span>
        </button>
      </div>
